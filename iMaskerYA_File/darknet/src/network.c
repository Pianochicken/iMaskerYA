...
char *detection_to_json(detection *dets, int nboxes, int classes, char **names, long long int frame_id, char *filename)
{
    ...
    // sprintf(time_date, "%d_%d_%d.txt", 1900+p->tm_year, 1+p->tm_mon, p->tm_mday);
    sprintf(time_date, "%d_%d_%d.txt", 1900+q->tm_year, 1+q->tm_mon, q->tm_mday);

    // sprintf(time_string, "%d_%d_%d %d:%d:%d", 1900+p->tm_year, 1+p->tm_mon, p->tm_mday, 8+p->tm_hour, p->tm_min, p->tm_sec);
    sprintf(time_string, "%d_%d_%d %d:%d:%d", 1900+q->tm_year, 1+q->tm_mon, q->tm_mday, q->tm_hour, q->tm_min, q->tm_sec);

    // strcat(send_buf, "\n ] \n}"); // original
    strcat(send_buf, "\n ],\n \"time_string\":\"");
    strcat(send_buf, time_string);
    strcat(send_buf, "\"\n},\n");
    // int length = strlen(send_buf);
    // printf("length: %d, size: %d \n", strlen(send_buf), sizeof(send_buf));

    if (frame_id % 10 == 1) {

        // printf("frame_id: ", frame_id);

        FILE *fp_cureent;
        fp_cureent = fopen("current.txt", "w+");
        fwrite(send_buf, 1, strlen(send_buf), fp_cureent);
        fclose(fp_cureent);

        FILE *fp_date;
        fp_date = fopen(time_date, "a");
        fwrite(send_buf, 1, strlen(send_buf), fp_date);
        fclose(fp_date);
